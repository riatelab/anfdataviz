...
*"La science est infaillible, mais les savants se trompent toujours."* (Anatole France, 1889). Ce principe peut également être appliqué à la cartographie. En effet, toute carte est issue d'un processus complexe de choix, de selections, d'opérations statistiques ou géomatiques. Certains auteurs énoncent que les cartes sont subjectives. D'autres auteurs disent carrément que les cartes mentent. Quoi qu'il en soit, toute carte résulte de l'acte créateur des des choix de son auteur. Donc quand on se situe dans une démarche scientifique, ces choix doivent être traçables, partageables et soumis à la discussion scientifique (ce qui est difficilement faisable quand ces cartes sont réalisées dans un environnement "clic-bouton"). 

La réalisation des cartes dans le langage R permet donc de tracer toutes les opérations de la réalisation cartographique. Réaliser des cartes dans ce langage unique permet, en diffusant le code source en même temps que les cartes, de jouer "carte sur table". Cela permet de détailler les choix qui ont été faits, et de s'exposer à la controverse scientifique.  Cela permet aussi de travailler à plusieur sur une carte en associant des compétances complémentaires (sémiologie graphique, statistiques, géomatique, etc.) et de faciliter la mise à jour de documents déjà rélisés (en rééxecutant un code préalablement réalisé).
...
sur aux autres. Cela ne nuit pas à la lisibilité générale de la carte mais on peut
...
Pourquoi ne pourquoi choisir des carrés plutôt que des cercles ? 
...
Astuce ! Le package *eurostat* permet d'accéder aux données de statistiques
européennes directement dans l'environnement de R sans avoir besoin 
d'aller chercher sur le site Web et de les télécharger. Cela permet 
d'avoir des données à jour et d'éviter toute erreur de manipulation de 
données. 
...
On choisit donc la méthode des quantiles. 
...
Ces difficultés liées aux hétérogénéités de maillage renvoient une série 
...
souvent illustré par ce type d'illustration. 
...
ce quadrillage régulier au prorata de la surface représentée. 
...
les cartes lissées (ou de potentiel) qui consiste à
s'affranchir complètement du maillage administratif sous-jacent. 
