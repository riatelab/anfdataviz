...
<img src ="pics/construction.jpg"></img>
...
Pour exécuter les programmes proposés par la formation, [installez / mettez à jour R](https://www.r-project.org/){target="_blank"} (version 3.4.4 minimum), [R Studio](https://www.rstudio.com/){target="_blank"}. Lancez ensuite R studio et installez les packages au moyen des commandes suivantes (prévoir bien 10 minutes) 
...
Les commandes de base si dessous permettent de visualiser le nom des colonnes,
...
Ce package permet de créer et intégrer des cartes thématiques dans sa chaine de 
traitement en R. Il permet des représentations cartographiques tels que les cartes
en symboles proportionnels, des cartes choroplèthes, des typologies, des cartes de
flux ou des cartes de discontinuité. Il offre également des fonctions qui permettent
...
accessible directement dans R Studio. Pour celà, vous pouvez taper simplement :
...
qui présente intelligemment les packages *sf* et *cartography* à partir d'un cas d'utilisation
centré sur Mexico. 
...
les différentes valeurs du tableau de données.
...
en figuré proportionnel grâce à la fonction *propSymbolsTypoLayer* :  
...
A cette échelle géographique on remarque que les cercles se surimposent les uns
aux autres. Cela ne nuit pas à la lisibilité générale de la carte mais on peut
aussi décider d'utiliser un package et une fonction qui permet d'éviter cela. 
...
On souhaite réaliser une carte sur le chômage des ouvriers en Occitanie. Réalisez une 
carte avec les parametres suivants :
...
Il s'agit des géométries de référence des territoires européens adaptés à 
la cartographie thématique proposés par [l'UMS RIATE](http://riate.cnrs.fr/?page_id=153). 

```{r, eval=T}
# Fond de carte principal
regions <- st_read(dsn = "data/GREAT/GEOM_EU34_NUTS13/geom_eu34_nuts13.shp", stringsAsFactors = F) 

# Elements d'habillage
borders <- st_read("data/GREAT/OTHERS/borders_EU34.shp", stringsAsFactors = F)
countries <- st_read("data/GREAT/OTHERS/background_countries.shp",stringsAsFactors = F)
coastlines <- st_read("data/GREAT/OTHERS/coast_wide.shp", stringsAsFactors = F)
cyprus <- st_read("data/GREAT/OTHERS/cyprus_north_part.shp", stringsAsFactors = F)
remote <- st_read("data/GREAT/OTHERS/remote_territories_wide.shp", stringsAsFactors = F)
seaboxes <- st_read ("data/GREAT/OTHERS/remote_area_background_wide.shp",stringsAsFactors = F)
background <- st_read("data/GREAT/OTHERS/background.shp", stringsAsFactors = F)
```

Il est toujours utile de visualiser les géométries utilisées et consulter la table attributaire
...
Ce bloc de code est dédié est à la recolte des données mises à disposition par Eurostat.
Le package *eurostat* permet de les extraire. Le package *reshape2* sert à mettre
en forme les données. La procédure à suivre est la suivante :

1. Consultez le contenu de la base de données [Eurostat](https://ec.europa.eu/eurostat/fr/data/database){target="_blank"} 
et identifiez la table qui contient vos indicateurs d'intérêt (cf ci-dessous)
2. Importez les packages *eurostat* et *reshape2*.  
...
5. Filtrez les dimensions du tibble pour réduire son contenu à deux dimensions
(objets géographiques * indicateur par exemple).
6. Transformez le tibble en dataframe avec la fonction *dcast*. Dans notre cas de figure
nous conservons en ligne les objets géographiques et en colonne la dimension temporelle
de l'indicateur d'intérêt. 
7. Quand tous les indicateurs ont été téléchargés et préparés, les joindre aux
géométries des régions européennes (regions dans notre cas de figure). 

<img src = "pics/ESTAT.png", alt = "Base de données régionale d'Eurostat></img>
...
Dans cartography, plusieurs méthodes de discrétisation sont proposées. Jetez un
oeil à la documentation de la fonction getBreaks pour en savoir plus. 

On peut maintenant observer avec un graphique simple les effets de plusieurs
méthodes de discrétisation sur notre variable de densité de population. 

Compte-tenu du nombre d'unités territoriales NUTS3 en Europe, nous choisissons
10 classes pour guider la discrétisation. 
...
Tout est prêt ! Cartographions cet indicateur avec la méthode des quantiles,
la palette "red" en 10 classes. Associons des couches d'habillage. Et mettons en 
page la carte (flèche du nord, échelle, sources) grâce à l'instruction layoutLayer
de cartography. 
...
La méthode du carroyage consiste à découper l'espace géographique en un maillage
formé de carrés réguliers dans une projection donnée. Elle permet de s'affranchir
de l'arbitraire et de l'irrégularité d'un découpage administratif, comme c'est
ici le cas avec l'Europe au niveau NUTS3. La donnée est répartie sur ce quadrillage
régulier au prorate de la surface représentée. 

<img src = "pics/carroyages.png"></img>

Comment faire ? L'instruction getGridLayer du package cartography permet de construire ces grilles 
...
Créez en vis à vis une carte choroplèthe et une carte carroyée avec les paramètres suivants :
...
Pour la carte carroyée
* Résolution de la carte carroyée : 100 km
* Type de grille : hexagones

Exécutez ce code qui permet au prélable de préparer la grille et retirer les valeurs
manquantes qui nuiraient au calcul.

```{r, eval=T}
# Retirer les valeurs manquantes
regions <- regions[!is.na(regions$GDP_2014),]
regions <- regions[!is.na(regions$POP_2014),]
...
regions$GDP_HAB <- regions$GDP_2014 * 1000000 / regions$POP_2014
```
...
Téléchargez les doonnées ici : [Télécharger](https://github.com/riatelab/anfdataviz/blob/master/exercice4.zip){target="_blank"}
